...
func postTask(w http.ResponseWriter, r *http.Request) {
	...
	// Поскольку мы присваиваем id на стороне клиента, добавим проверку на его уникальность, без этого метод может перезаписать заметку с таким же id
	_, ok := tasks[task.ID]
	if ok {
...
func getTask(w http.ResponseWriter, r *http.Request) {
	...
	task, ok := tasks[id]
	if !ok {
		// по заданию, если задача не найдена - 400 Bad Request
...
////// https://www.postman.com/timur-tikhomirov/workspace/yandex/collection/17799631-caa39913-98c6-403b-af66-24ac2b5418a3?action=share&creator=17799631
